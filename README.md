...
- **Email/password auth scaffolding; the auth guard ships OPEN by default** (`APP_LAYOUT.authGuard`) — a fresh clone is the showcase-and-launchpad until you build in it, and consumers that never use login skip it freely. Flip the switch to wall authenticated surfaces. Consumers needing PIN+device-UUID auth re-add the 4 PIN primitives + `audit-rpc-auth.ts` + a `verify_session` RPC as a customization (see `CLAUDE.md` → "When to add PIN auth").
