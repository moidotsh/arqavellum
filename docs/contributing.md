...
- **Keep `showcase` out of the MobilePremium barrel.** `components/MobilePremium/index.ts` must not re-export `showcase`. Doing so re-introduces a four-step require cycle (primitives barrel → LoadingOverlay → MobilePremium barrel → showcase → primitives barrel) that Metro warns about. The single consumer is `app/dev/premium.tsx`, which imports it directly from `./components/MobilePremium/showcase`.
