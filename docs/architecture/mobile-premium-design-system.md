...
| `useAnimatedValue` / `animateTo` / `useTransition` / `useAnimatedFlag` / `useLoop` | The kit's one Animated engine (`useAnimatedValue.ts`): ref boilerplate, imperative one-shots, effect-shaped transitions, boolean flags (snapping under reduced motion), and ambient loops (never started under reduced motion). Every timed/sprung/looping value in the kit routes through it so reduced-motion clamping, stop-cleanup, and web/native driver correctness live in one place. |
| `useFieldChrome` / `FIELD_GROUP_STYLE` | The form trio's shared chrome mapping (border / background / label color respond to focus + error) and the shared label+control group layout. |
| `useDismissOnEscape` | Escape-to-dismiss for portal surfaces (dialog, sheet) — web-only, paired listener cleanup. |
...
| `MobileSelect` | Bottom-sheet selector with a 54px trigger mirroring MobileInput's styling. The sheet is the canonical `MobileSheet` (dialog role, focus return, Escape-to-close, backdrop tap, 70% height cap) — MobileSelect owns only the trigger and the option rows. The trigger group spreads the shared `FIELD_GROUP_STYLE` (the width policy + form rhythm) and the border/background/label mapping comes from `useFieldChrome` — one rhythm across the form trio. |
...
| `CopyForAiButton` | Dev-helper button for the "Copy for AI" pattern (CLAUDE.md invariant 11). Drops into `MobileHeader.navRightAction` or `MobileHomeHeader.rightAction`; accepts a pre-built `payload` string (from `buildAiPayload` in `utils/`) and owns the visual treatment, clipboard call, success/failure toast, and logger calls. Two variants: `'ghost'` (default — the compact nav-mode header row) and `'subtle'` (brand-tinted chip for the home header). |
...
| `AppShellHeader` (composed) | The one-drawer pattern pre-assembled in `components/composed/`: `MobileHomeHeader` (brand from `APP_DISPLAY_NAME`) + `HamburgerButton` + `MobileNavDrawer` in the `APP_LAYOUT` persistence/anchor defaults, drawer locked to `MOBILE_CONTENT_MAX_WIDTH`, chrome dialect-aware: the `MobileNavDrawerGlassCap` layers under the masthead under `glass`, while under `ink` the cap retires, the masthead rides the plate (`onPlate` + paper-flipped trigger), and destinations render as ledger rows. Consumers pass `items` (navigateTo* helpers from the navigation barrel) plus optional `subtitle` / `rightAction` / `footer` / `atmosphere` slots — no hand-wiring of the cap/anchor/columnWidth triple. Belongs to the home surface only; child screens carry the `> [title]` `MobileHeader`. |
| `MobileActionFooter` | Sticky bottom action area. Holds the primary + optional secondary. 76–92px total with safe-area bottom. |
| `MobileTabBar` | The kit's bottom chrome — flat card bar with a 1px hairline top edge, four flanking tabs plus a raised 56px center-action circle (the app's primary verb; `active` runs a 1.6s opacity/scale breath that collapses under reduced motion). Active tab = ink label + 2px top-edge notch; labels ride the mono face when declared. Caps to the mobile content column; the center slot stretches the row so the raised action actually raises. Owns no routing — consumers pass `onPress` callbacks (their NavigationHelper). |
...
| `ActivityGridPreview` | Dev-only preview host for `ActivityGrid` (the showcase's vehicle for it) — demonstrates both layout modes, the `columns` prop with a live selector, and the four states a consumer designs for: populated, empty (valid range), invalid range, responsive-matrix auto-sizing. Sample data is synthetic and deterministic; not a production component. |
...
| `DisclosureRow` | Expand/collapse row with consumer-supplied header + content. v1 motion contract: **instant content + rotating chevron** — content appears/disappears with no height animation; chevron rotates 180° on open over 200ms via `useAnimatedFlag` (the kit's motion core), snapping under `prefers-reduced-motion`. Height animation is a Batch B concern gated on a Reanimated adoption decision plus a measurement helper that doesn't exist today. Header wrapper carries `role="button"` + `accessibilityState={{ expanded }}`; chevron is decorative (`accessibilityElementsHidden`). Supports controlled (`open` + `onOpenChange`) and uncontrolled (`defaultOpen`) usage. |
...
- **C1 router calls** — Never `router.push/replace/back` directly. Arqavellum ships `navigation/NavigationHelper.tsx`; consumers extend it with domain routes (see `CLAUDE.md` → "How to consume" step 9). The `MobileHeader` `onBack` prop takes a `() => void`; wrap async helpers with `() => { void safeGoBack(); }`.
- **C4 ActivityIndicator** — Don't use RN's `ActivityIndicator` directly. Four sanctioned sites: `components/primitives/{LoadingSpinner,LoadingOverlay,AppLoading}.tsx` (all shipped) and `MobilePrimaryButton`'s inline spinner (`c4-exempt` in `MobilePrimaryButton.tsx` — reuse it via the `loading` prop). Consumers wrap these primitives for domain loading states.
...
**Desktop layout policy (`DESKTOP_LAYOUT_MODE`, `constants/styles.ts`):** `'mobile-only'` (default) shelves every per-screen desktop lift — `ScreenScaffold`'s `bodyMaxWidth` prop and the desktop-breakpoint branches — so the constrained mobile column is the layout at ANY viewport width, and the lift call sites stay in place untouched for when the desktop layouts come back. `'multi-column'` re-enables them all: screens may lift the body cap and branch multi-column layouts on the desktop breakpoint. One constant flips the policy, same lockstep discipline as `CONTENT_WIDTH_MODE`; flipping it is the consumer-side signal to adopt a real desktop layout strategy (see `docs/contributing.md` → "Adding desktop support"), not a per-screen escape hatch.
