...
5. **Override the `brand` color slot** in `constants/theme.ts` → `theme.colors.light.brand` (and `brandHover`, `brandPress`, `brandMuted`, `brandSoft`, `buttonBackground`, `buttonBackgroundDisabled`), plus `brandText` (both modes) and — if the brand is bright rather than dark — `textOnBrand`, per invariant 5's AA rules. Also set the product NAME in `constants/displayName.ts` (`APP_DISPLAY_NAME`) — the auth footer, home placeholder, and PWA meta titles all read it.
