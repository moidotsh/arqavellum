...
4. **Auth scaffolding ships; the guard ships OPEN by default.** The full email/password stack is wired — auth screens, `AuthService`, the auth store — but a fresh clone renders every route for every visitor (`APP_LAYOUT.authGuard = false` in `constants/layout.ts`): the starter is a showcase-and-launchpad until the consumer builds in it, and plenty of consumers never use login at all. A consumer with authenticated surfaces flips `authGuard: true` and `AuthGuard` (`enabled` prop, wired in `_layout.tsx`) walls everything except `PUBLIC_SEGMENTS` (`dev`, `qr`) and the not-found exemption again — the signed-in-away-from-auth-screens redirect runs in both modes. Arqavellum ships no PIN primitives (`MobilePinInput`, `MobileFullPagePinEntry`, `MobilePinReauthSheet`, `_PinKeypad`) and no `audit-rpc-auth.ts` (the R1 RPC `verify_session` audit). A consumer needing PIN+device-UUID auth re-adds those four files + that one audit script as a customization — see "How to consume" below.
5. **The `brand` color slot is the single override point.** Arqavellum defaults to a neutral indigo (`#4F46E5`). Consumers override `theme.colors.light.brand` (and the related `brandHover` / `brandPress` / `brandMuted` / `brandSoft` / `buttonBackground` / `buttonBackgroundDisabled` keys, plus `brandText` — the AA text companion: `brand` is a fill/large-type slot at 3:1, small brand text (10–15px labels, eyebrows, links) reads `brandText`, and a saturated brand hue darkens that slot in light mode / brightens it in dark until it clears 4.5:1; a brand bright enough that white fails on the fill also overrides `textOnBrand` to near-black ink). Palette retunes are **measured AA on the darkest surface the token rides** (`backgroundDeep` light / `card` dark) — see design-system §9.4's table. Don't introduce a second "accent" slot — every consumer wins from one canonical override. The surface-language family groups under **`theme.dialect`** (`'glass' | 'ink'`): ONE declaration presets the atmosphere, the drawer, the toast (`theme.toast.style`: `'card' | 'chit'` — the bordered-card default vs the ink chit: ink plate, paper type, one status dot, receipt mono when a mono face is declared), and the transition axis (`theme.transition.style`: `'none' | 'curtain'` — `'none'` (the glass preset) retires the machinery entirely; `'curtain'` (the ink preset) mounts the shell's `RouteCurtain` — the ink-plate/paper-chaser navigation transition (`components/MobilePremium/RouteCurtain.tsx` + `utils/routeTransition.ts`), wired through NavigationHelper's `withRouteCurtain` seam, stamping the destination's registry title; unmounted and pass-through under `'none'`, web-only, never in DOM tests, never under reduced motion); per-point literals override the preset for deliberately mixed taste. Type families follow the same discipline: **`theme.fonts`** (`display?` / `mono?`, both undefined by default — `mobileTitle`/`mobileEyebrow` and the figure styles pick a face up only when one is declared; see the `TypeFaces` self-hosting recipe in `constants/theme.ts`). Shape has the same discipline: **`theme.shapes`** (surface / sheet / control / tile / tag) is the single shape-language override point, consumed by every kit primitive — a consumer retunes the family (e.g. sleek/monotone: `surface: 8, tag: 4`) without touching component files. Atmosphere too: **`theme.atmosphere`** (`style: 'aurora' | 'flat'`) is the single atmosphere-language override point — `'flat'` turns the drifting orbs off app-wide (base tint + vignette stay, every `MobileAtmosphere` including the nav drawer follows) with no per-callsite prop threading; `showOrbs` / `atmosphereShowOrbs` remain as explicit per-callsite overrides for the showcase. The nav drawer follows the same discipline: **`theme.drawer.style`** (`'sheet' | 'ink'`) — `'sheet'` is the glass-scrim default; `'ink'` renders the `InkPanel` body (`components/MobilePremium/InkPanel.tsx` — the inverted-surface primitive: text-color plate + print grain + brand edge rule) with cutout or slideout brand persistence intact (in cutout the plate runs the panel's full height and the masthead rides ON it — `MobileHomeHeader onPlate` stacks the real header above the drawer, its type bleeding to the background color), a flat dim scrim (no blur), Space-Mono-friendly rows via the `itemLabelStyle` slot, and the out-cubic slide curve.
...
13. **Web analytics is a disabled-by-default seam; `/qr` is the printed-QR landing route.** `utils/webAnalytics.ts` is the official no-framework Vercel Web Analytics snippet — a `window.va`/`vaq` queue stub plus the `/_vercel/insights/script.js` tag injected from the root layout's third web-only effect — deliberately NOT the `@vercel/analytics` package (Metro never resolves a DOM-only dependency; the shell adds no dependency). The seam ships DISABLED: `EXPO_PUBLIC_WEB_ANALYTICS=1` (consumer env) gates BOTH script injections — Web Analytics AND Speed Insights (`initSpeedInsights`: a `window.si`/`siq` queue stub plus its own `/_vercel/speed-insights/script.js` tag with `data-sdkn`/`data-sdkv`, hand-rolled to mirror @vercel/speed-insights@2.0.0's generic inject — the npm package and its DOM-only deps never enter the bundle; the consumer must also enable the products in the Vercel dashboard); visit-source classification (`qr` > `pwa` > `link` > `internal` > `direct` — path, standalone display-mode, referrer origin) always runs locally and reports once per session (`arqavellum-visit-source` in sessionStorage). The landing-path SET is `QR_LANDING_PATHS` (`/qr` ships; consumers forking attribution coin more 17-byte paths, stub their routes, extend the array, and never remove a legacy entry — printed codes stay in the wild). `app/qr.tsx` (sb1-exempt redirect stub, router-public) is the landing for printed QR codes: consumers print `https://<their-domain>/qr`, kept at or under QR version 1-L's byte-mode capacity (17 bytes — no parameters, ever; one more character errors the encoder and a denser grid scans worse). The qr screen reports before its redirect — child effects run before the root layout's, and the session flag makes either order report exactly once. Consumers enabling analytics must also enable Web Analytics in the Vercel dashboard and redeploy (until both, the script 404s — harmless; `/qr` pageviews and the Referrers "Direct" bucket read on any dashboard plan, `visit_*` custom events need Pro). Classifier goldens in `__tests__/utils/webAnalytics.test.ts`; the redirect smoke in `__tests__/navigation/qr.test.tsx`; `qr.html` is in `verify-web-build`'s required set. **The print kit ships**: `scripts/generate-print-kit.ts` (zero deps, writes gitignored `_print/print-kit.html` — poster + sticker sheets) encodes the landing URL with a from-scratch QR v1-L encoder verified mask-for-mask against an independent reference; capacity is EXACTLY 17 bytes and one more character errors, so the smallest-scannable-grid rule is a property of the tool. The URL comes from `PRINT_QR_URL` (env) — the tool refuses to run without it. Goldens in `__tests__/scripts/print-kit.test.ts`.

14. **The i18n seam ships opt-in; the starter's screens stay English.** `utils/i18n.ts` is the typed-catalog harness for consumers shipping more than one language: ONE source language owns the truth, every other language is a MIRROR typed as the source's shape (`createStringsCatalog(source, mirrors)` — an untranslated string is a compile error, never a runtime blank), `t(lang)` is pure/golden-testable, `useT()` subscribes the persisted lang store (`arqavellum-lang`) so every mounted catalog surface re-renders in place on toggle, `resolveLang` normalizes corrupt persisted values back to `DEFAULT_LANG`, and `setLang`/`initDocumentLang` mirror the live language onto `<html lang>`. `LangToggle` (MobilePremium) is the one-gesture pill. Content is not chrome: catalogs carry UI strings only — people's words and product names never translate. Consumers edit the `Lang` union + `DEFAULT_LANG` in the same change as their first slice; the type error list is the migration map. (Storage adapter note: `zustandStorage` lives in `utils/storage.ts` — utils modules import it at the LEAF, never via the stores barrel, which would close a barrel cycle.)
...
2. **Find-and-replace `arqavellum` → `my-app`** in:
   ...
   - `constants/storageKeys.ts` — every runtime literal that carries the starter's name (persisted storage keys, injected DOM ids, the dev-bypass email) is centralized here; without this pass, stale `arqavellum-*` keys linger in DevTools storage and two clones on the same localhost origin share localStorage.
   - This file (title + headings)
3. **Set up env:** copy `.env.local.example` → `.env.local`, fill in your Supabase URL + anon key.
4. **Pick your gating posture.** The starter opens, not walls: `APP_LAYOUT.authGuard` (`constants/layout.ts`) ships `false`, so every route renders for every visitor and `app/index.tsx` is a launchpad — it links the design-system showcase whenever dev surfaces are real (`EXPO_PUBLIC_DEV_SURFACES=1`, set by the `start`/`web` scripts; production exports stub `/dev/*` and the launchpad hides the row) and offers Sign in for the auth scaffolding. A consumer with authenticated surfaces flips `authGuard: true` — the guard then walls everything except `dev`, `qr`, and the not-found page. Consumers that never use login can delete the auth screens + `useAuthNavigation` freely; the shell doesn't depend on them.
5. **Replace icons (two surfaces):** `public/icons/*` (PWA, always required — 192.png, 512.png, 512-maskable.png) and `assets/*` (native extension — icon.png, splash-icon.png, adaptive-icon.png). Arqavellum ships neutral indigo placeholders for both; the consumer overrides with brand artwork. The `assets/*` PNGs only matter for a native build, but replacing them upfront is cheap. Before any native release also replace the `ios.bundleIdentifier: 'app.arqavellum'` starter value in `app.config.ts` + `app.json` with your own iOS bundle ID and add your Android application/package ID — `app.arqavellum` is starter scaffolding, not a consumer's release identity.
6. **Override the `brand` color slot** in `constants/theme.ts` → `theme.colors.light.brand` (and `brandHover`, `brandPress`, `brandMuted`, `brandSoft`, `buttonBackground`, `buttonBackgroundDisabled`), plus `brandText` (both modes) and — if the brand is bright rather than dark — `textOnBrand`, per invariant 5's AA rules. Also set the product NAME in `constants/displayName.ts` (`APP_DISPLAY_NAME`) — the auth footer, home placeholder, and PWA meta titles all read it.
7. **(Optional) customize the atmosphere language + palettes.** The background style is one declaration: `theme.atmosphere.style` in `constants/theme.ts` — `'aurora'` (default, drifting orbs) or `'flat'` (base tint + vignette only; every `MobileAtmosphere`, the nav drawer, and the auth screens follow with no per-callsite props). The 7 semantic surface palettes are domain-agnostic but a consumer may want different hues — edit `components/premium/shared/atmospherePalettes.ts` (copy-then-customize).
8. **Drop in domain code:**
   - Routes → `app/` (replace the launchpad `app/index.tsx`)
   - Stores → `stores/`
...
9. **Extend the navigation helper:** arqavellum ships `navigation/NavigationHelper.tsx` with a small route set (`NavigationPath` enum + `navigationHierarchy` map + `navigateTo*` / `replaceWith*` helpers). Audit C1 allows `router.push/replace/back` only here and in `hooks/useAuthNavigation.ts`. When you add domain routes:
...
10. **Loading — three stories, one rule:** (a) inline/loading-in-a-row → `LoadingSpinner` (`components/primitives/`); (b) full-screen blocking → `LoadingOverlay` (same folder; used by the showcase) or `AppLoading` for the boot splash; (c) in-button → `MobilePrimaryButton`'s `loading` prop. For content-shaped placeholders (cards, lists, receipt rows) compose `SkeletonBlock` (MobilePremium) — it pulses via `useShimmer` and never touches `ActivityIndicator`. Audit C4 sanctions exactly four `ActivityIndicator` sites: the three primitives files plus the button spinner. Add domain-specific loading wrappers as composed components that import these primitives — don't sprinkling `ActivityIndicator` directly into domain code.
