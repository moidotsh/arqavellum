...
**The icon shim is load-bearing build plumbing (perf):** metro.config.js resolves the exact specifier `@tamagui/lucide-icons-2` to `shims/lucide-icons-2.js`, GENERATED by `scripts/sync-icon-shim.ts` from the repo's actual imports (chained before every expo command; metro.config regenerates it if missing). The CJS nuclear option (zustand's `import.meta`, see metro.config.js) defeats barrel tree-shaking — importing one icon pulls all 3520 (measured on this repo's export: 5.37MB → 3.91MB raw, −27%, for 27 icons). The shim is auto-synced, so adding an icon import just works; a typo'd icon name fails the sync loudly instead of shipping a blank glyph.
