...
// ICON SHIM: the CJS resolution above defeats tree-shaking for the
// @tamagui/lucide-icons-2 barrel — importing one icon pulls all 3520
// (over a megabyte of minified bundle for a few dozen used). The exact
// barrel specifier resolves to shims/lucide-icons-2.js, GENERATED by
// scripts/sync-icon-shim.ts from the repo's actual imports (chained
// before every expo command in package.json). If the shim is missing
// — someone ran the expo CLI directly — regenerate it here so no
...
if (!existsSync(LUCIDE_SHIM)) {
  spawnSync('bun', ['run', 'scripts/sync-icon-shim.ts'], { stdio: 'inherit', cwd: __dirname });
}
const defaultResolveRequest = config.resolver.resolveRequest;
config.resolver.resolveRequest = (context, moduleName, platform) => {
  if (moduleName === '@tamagui/lucide-icons-2') {
    return { type: 'sourceFile', filePath: LUCIDE_SHIM };
  }
